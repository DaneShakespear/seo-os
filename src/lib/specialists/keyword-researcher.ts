  ...
  async execute(ctx) {
    ...
    await updateCanonicalNote(
      ...
      [
        ...
        "Provider-reported values are shown per query row. Close variants may share the same value; treat repeated values as one demand cluster and do not add them together.",
        "Approved URL rules override heuristic matching. The homepage owns the explicitly approved primary limo-service terms; `/services/` owns broader service-category and comparison variants.",
        "",
        keywordTable,
        "",

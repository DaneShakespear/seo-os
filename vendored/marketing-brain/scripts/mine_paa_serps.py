...
def _write_digest(path: Path, paa_data: list[dict[str, Any]], related_data: list[dict[str, Any]]) -> None:
    ...
    lines.append("  - paa")
    has_findings = bool(question_counter or related_counter)
    lines.append(f"status: {'active' if has_findings else 'empty'}")
    lines.append(f"confidence: {'high' if has_findings else 'low'}")
    lines.append(f"approval_status: {'approved' if has_findings else 'needs-review'}")
